...
const CustomerName = tw.h5`font-bold text-lg lg:text-xl xl:text-2xl text-appcolor`;
const CustomerTitle = tw.p`font-medium text-sm`;

const QuotesLeft = tw(
  QuotesLeftIcon
)`w-8 h-8 lg:w-10 lg:h-10 text-appcolor absolute top-0 left-0`;
const QuotesRight = tw(
  QuotesRightIcon
)`w-8 h-8 lg:w-10 lg:h-10 text-appcolor absolute bottom-0 right-0`;

const SliderControlButtonContainer = styled.div`
  ${tw`absolute top-0 h-full flex items-end md:items-center z-20`}
  button {
    ${tw`text-secondary-500 hover:text-appcolor focus:outline-none transition duration-300 transform hover:scale-125 transform -translate-y-2/3 md:translate-y-0`}
    svg {
      ${tw`w-8`}
    }
  }
...
const DecoratorBlob1 = tw(
  SvgDecoratorBlob1
)`absolute w-32 top-0 left-0 -z-10 text-appcolor opacity-25 transform -translate-x-full`;
  ...
  const testimonials = [
    {
      imageSrc: "",
      quote:
        "As a divorcee, Sanda Uwargida has made it possible for me to send my children to school and take care of my home",
      customerName: "Hajara",
      customerTitle: "",
    },
  ];
