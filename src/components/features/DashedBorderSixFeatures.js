...
const Card = styled.div`
  ${tw`flex flex-col mx-auto max-w-xs items-center px-6 py-10 border-2 border-dashed border-appcolor rounded-lg mt-12`}
  .imageContainer {
    ${tw`border-2 border-appcolor text-center rounded-full p-6 flex-shrink-0 relative`}
    img {
      ${tw`w-8 h-8`}
    }
  }
  ...
  .title {
    ${tw`mt-2 font-bold text-xl leading-none text-appcolor`}
  }
  ...
  return (
    <Container>
      <ThreeColumnContainer>
        <Heading>Our Professional <span tw="text-appcolor">Services</span></Heading>
        {cards.map((card, i) => (
          <Column key={i}>
            <Card>
